...
const APIFetchAndSearch = () => {
  ...
  return (
    <section className="w-full min-h-screen bg-indigo-800 flex flex-col items-center justify-center p-5">
      <header className="w-[90%] h-[10vh] bg-white rounded-full shadow flex justify-between px-10">
        ...
        <div className="w-[40%] h-full flex justify-evenly items-center ">
          <aside className="w-[50%] h-full flex items-center justify-center relative">
            <input
              type="text"
              name="searchText"
              id="searchText"
              className="border p-2 rounded-full w-full outline-none focus:ring-1 focus:ring-indigo-700 transition pl-11 placeholder:text-gray-700"
              placeholder="Search by username or id"
              value={searchText}
              onChange={(e) => setSearchText(e.target.value)}
            />
            <span className="absolute top-6 left-4 text-2xl">
              <CiSearch />
            </span>
          </aside>
          <div className="w-[25%] flex gap-1">
            <span className="text-red-600 font-semibold">
              {filteredUsers.length}
            </span>
            <span className="text-indigo-600 font-semibold">Users Found</span>
          </div>
        </div>
      </header>
        ...
        {filteredUsers.length <= 0 ? (
          <>
            <p className="text-2xl font-semibold text-white">
              For this
              <span className="text-white font-semibold">{searchText}</span> no
              User Profile Found!
            </p>
          </>
        ) : (
